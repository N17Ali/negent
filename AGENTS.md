...
Cloudflare Worker (TypeScript) running a Telegram bot that aggregates RSS tech news, uses DeepSeek (via the NVIDIA API) to pick the best stories, then summarizes/translates them to Persian via Gemini, driven by 2 cron triggers. D1 (SQLite) is the only store. Zero runtime dependencies. `PLAN.md` has extra design context (free-tier budget, message format, error-handling rationale) — but trust the code where they disagree.
...
- Secrets are **not** in the repo. Set remotely via `wrangler secret put BOT_TOKEN`, `wrangler secret put GEMINI_API_KEY`, and `wrangler secret put NVIDIA_API_KEY`. For local dev, create a `.dev.vars` with `BOT_TOKEN=...` / `GEMINI_API_KEY=...` / `NVIDIA_API_KEY=...`.
...
- `30 5,8,11,14 * * *` → `cron/select.ts` (DeepSeek selects top 10 → Gemini summarizes → deliver to subscribers)
...
- **DeepSeek selection** (`services/deepseek.ts`): `deepseek-ai/deepseek-v4-pro` via `https://integrate.api.nvidia.com/v1/chat/completions`, `temperature: 1`. Strips ```` ```json ```` fences before parsing; retries with backoff on 429/503/524 (524 = NVIDIA timeout). Uses `NVIDIA_API_KEY`.
- **Gemini summarize/translate** (`services/gemini.ts`): primary `GEMINI_MODEL` (`gemini-3.1-flash-lite`); on rate-limit it falls back to `GEMMA_MODEL` (`gemma-4-31b-it`). Uses `GEMINI_API_KEY`.
...
- `src/types.ts` `Env` interface is the source of truth for bindings/secrets: `DB`, `BOT_TOKEN`, `GEMINI_API_KEY`, `NVIDIA_API_KEY`.
- All user-facing Telegram text is Persian, informal tone (use "تو" not "شما", keep tech terms in English). Prompt templates live in `src/services/gemini.ts` (summarize/translate) and `src/services/deepseek.ts` (selection).
- Tuning knobs (batch sizes, thresholds, delivery hours, keyword lists, model names) are centralized in `src/utils/constants.ts` (and `DEEPSEEK_MODEL` in `deepseek.ts`).
