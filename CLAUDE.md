...
`sources` → `fetch` cron pulls **one** source per run (rotated via `fetch_order`), filters with `utils/filter.ts` keyword lists, dedups by `url_hash`, inserts as `status='raw'` → `select` cron sends up to `BATCH_SELECT_SIZE` (150, newest first) raw titles to Gemini, which returns `SELECT_TOP_N` (3) picks plus a **bucket** of important runner-ups; the picks go to `selected`, the bucket stays `raw` (reconsidered next run), and everything else is marked `skipped` → Gemini summarizes/translates the picks into `done` articles (`delivered=0`) → the separate `deliver` cron ships **one** `done` article per tick: it sends the text **first** (marking `delivered=1` once it reaches ≥1 subscriber), then generates the Persian voice reading and sends it as a reply that quotes the text. Text-first is deliberate — a voice pass can take ~a minute and one Worker invocation's `ctx.waitUntil` tail window can't hold that (`waitUntil() ... cancelled`), so both the `scheduled` handler and the manual endpoints **`await`** the pipeline (never `waitUntil`) to keep the invocation alive, and delivery ships text before the slow audio pass so the summary is guaranteed and the voice stays pure best-effort.
